  ...
  return (
    <svg className="histogram" viewBox={`0 0 ${chartWidth} ${chartHeight}`} preserveAspectRatio="none">
      ...
      {data.map((value, index) => {
        ...
        return <rect key={index} x={x} y={margin.top + innerHeight - barHeight} width={Math.max(0.6, barWidth * 0.84)} height={barHeight} rx={0.5} />;
      })}
        ...
        ? yTicks.map((tick) => {
            ...
            return (
              <g key={`yt-${tick}`}>
                ...
                <text x={margin.left - 4} y={y + 2} textAnchor="end" className="plot-tick-label" style={{ fontSize: `${tickFont}px` }}>{formatAxisTick(tick)}</text>
              </g>
            );
          })
        : null}
        ...
        ? xTicks.map((tick) => {
            ...
            return (
              <g key={`xt-${tick}`}>
                ...
                <text x={x} y={chartHeight - 8} textAnchor="middle" className="plot-tick-label" style={{ fontSize: `${tickFont}px` }}>{Math.round(tick)}</text>
              </g>
            );
          })
        : null}
      {showXAxisLabel ? <text x={chartWidth / 2} y={chartHeight - 1.5} textAnchor="middle" className="plot-axis-label" style={{ fontSize: `${axisLabelFont}px` }}>ADC units</text> : null}
      {showYAxisLabel ? <text x={margin.left + 1} y={margin.top + 4} textAnchor="start" className="plot-axis-label" style={{ fontSize: `${axisLabelFont}px` }}>Counts</text> : null}
    </svg>
  );
}
...
export function LineSeries({
  ...
  if (data.length === 0) return <div className="line-empty">no points yet</div>;
  ...
  return (
    <svg className="line-series" viewBox={`0 0 ${chartWidth} ${chartHeight}`} preserveAspectRatio="none">
      ...
      {showYTicks ? yTicks.map((tick) => {
            const y = margin.top + innerHeight - (tick / max) * innerHeight;
            return <g key={`line-yt-${tick}`}><line x1={margin.left - 3} x2={margin.left} y1={y} y2={y} className="plot-axis" />
              <text x={margin.left - 4} y={y + 2} textAnchor="end" className="plot-tick-label" style={{ fontSize: `${tickFont}px` }}>{formatAxisTick(tick)}</text></g>;
          }) : null}
      {showXTicks ? xTicks.map((tick) => {
            const ratio = data.length <= 1 ? 0 : tick / (data.length - 1);
            const x = margin.left + ratio * innerWidth;
            return <g key={`line-xt-${tick}`}><line x1={x} x2={x} y1={margin.top + innerHeight} y2={margin.top + innerHeight + 2.5} className="plot-axis" />
              <text x={x} y={chartHeight - 8} textAnchor="middle" className="plot-tick-label" style={{ fontSize: `${tickFont}px` }}>{Math.round(tick)}</text></g>;
          }) : null}
      {showXAxisLabel ? <text x={chartWidth / 2} y={chartHeight - 1.5} textAnchor="middle" className="plot-axis-label" style={{ fontSize: `${axisLabelFont}px` }}>Time (windows)</text> : null}
      {showYAxisLabel ? <text x={margin.left + 1} y={margin.top + 4} textAnchor="start" className="plot-axis-label" style={{ fontSize: `${axisLabelFont}px` }}>Rate (Hz)</text> : null}
    </svg>
  );
}
